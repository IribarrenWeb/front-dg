...
export default {
    ...
    methods: {
        async onSubmit(){
            try {
                await service.update('audit', this.audit_id, { current_step: 10, valid_step: this.audit.valid_step >= this.currentStep ? this.audit.valid_step : this.currentStep, comprobations: this.comprobations})
                this.$emit('next', 9)
            } catch (err) {
