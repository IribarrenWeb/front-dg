	...
	<div>
		<form-validate @submit="submit" @keypress.enter.prevent="" v-slot="{ meta, resetForm }">
			...
			<template v-if="currentStep === 1">
				<div>
					<business-data class="mt-2" v-model:name="model.name" v-model:email="model.email"
						v-model:business_nif="model.business_nif" v-model:business_phone="model.business_phone"
							...
							<base-field name="delegate_id" label="Delegado" :required="true">
								...
								<div v-else>
									<field-validate name="delegate_id" label="Delegado" rules="required">
										<async-select customClass="custom-multiselect" @selected="delegate = $event"
					...
					<general-data v-model:property_name="model.property_name"
						v-model:property_last_name="model.property_last_name" v-model:property_dni="model.property_dni"
						v-model:property_phone="model.property_phone" v-model:property_email="model.property_email" />

					<address-select title="Dirección fiscal" v-model:address="model.address.address"
						v-model:city="model.address.city" v-model:code="model.address.code"
						v-model:country="model.address.country" v-model:province="model.address.province"
						v-model:comunity="model.address.comunity" v-model:street_number="model.address.street_number" />
				</div>
			</template>
			<template v-if="currentStep === 2">
				<responsible-data class="mb-2" v-model:responsible_name="model.responsible_name"
			...
			<template v-if="currentStep === 3">
				<fieldset v-for="(installation, id) in installations" :key="installation.key">
					<div class="border border-light rounded p-2 row mt-3">
						<installation-general-data v-model:file_document="installations[id].file_document"
							v-model:file_auditor="installations[id].file_auditor"
							v-model:auditable="installations[id].auditable_id" v-model:name="installations[id].name"
							v-model:periodicity="installations[id].periodicity"
							v-model:ministry_id="installations[id].ministry_id" :onlyForm="true" :form_id="id"
							:delegate_id="delegateId" />
						<div class="col-12">
							<address-select v-model:address="installations[id].address.address"
								v-model:city="installations[id].address.city" v-model:code="installations[id].address.code"
			...
			<template v-if="currentStep === 4">
				<fieldset class="border border-light rounded p-2 row mt-3" v-for="(installation, id) in installations"
					...
					<employee-general-data :form_id="id" custom_key="responsible" :isResponsible="true"
						...
						<div class="col-lg-6">
							<base-field :name="`installations.${id}.responsible.date_firm.date`" label="Fecha de alta">
								<field-validate type="date" class="form-control"
									:name="`installations.${id}.responsible.date_firm.date`" rules="" label="fecha de firma"
						...
						<div class="col-lg-6">
							<base-field :name="`installations.${id}.responsible.file_firm.base64`"
								...
								<field-validate v-else type="file" class="form-control"
									:name="`installations.${id}.responsible.file_firm.base64`" rules="" label="documento"
								...
								<div class="col-lg-6">
									<base-field apiName="responsible.date_certification" name="date_cer"
										...
										<field-validate type="date" class="form-control" name="date_cer"
											...
											label="fecha" v-model="installations[id].responsible.date_certification" />
									</base-field>
								</div>
								<div class="col-lg-6">
