<template>
	<div>
		<form-validate v-if="canShow" v-slot="{ meta }" @submit="onUpdate">
			<base-steps v-if="!profile" :currentStep="currentStep" listClasses="mb-md-4 pb-md-2" :steps="formatSteps"
				:meta="meta" :edit="true" @step="currentStep = $event" @navigate="currentStep = $event"></base-steps>
			<template v-if="currentStep === 1 || profile">
				<div>
					<business-data v-model:name="model.name" v-model:email="model.email"
						v-model:business_nif="model.business_nif" v-model:business_phone="model.business_phone"
						v-model:no_adr="model.no_adr" :update="true" />

					<general-data class="mb-2" v-model:property_name="model.property_name"
					...
					<AddressSelect title="Dirección fiscal" v-model:address="model.address.address"
			...
			<template v-if="currentStep === 2 || profile">
				<responsible-data class="mb-3" v-model:responsible_name="model.responsible_name"
			...
			<template v-if="currentStep === 3 && business?.user?.is_billable">
			...
			<div class="row mx-0 justify-content-end">
				...
				<base-button class="ml-2" type="default" @click="currentStep--"
					v-if="currentStep !== 1 && !profile">Anterior</base-button>
				<base-button v-if="!profile" type="default" :outline="true" class="ml-2"
					@click="this.$emit('close')">Cancelar
				</base-button>
			</div>
		</form-validate>
	</div>
...
export default {
	...
	components: { AddressSelect, GeneralData, BusinessData, ResponsibleData, BankData, SubscriptionModule }
}
</script>,
