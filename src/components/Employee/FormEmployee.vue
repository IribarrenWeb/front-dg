<template>
	<div>
		<base-steps v-if="employee_id" :currentStep="currentStep" listClasses="mb-md-4 pb-md-2" :steps="steps"
			:edit="true" @step="currentStep = $event" @navigate="currentStep = $event"></base-steps>
		<template v-if="currentStep == 1 || !employee_id">
			<form-validate ref="form" @submit="onSubmit" v-slot="{ resetForm, meta }" v-if="canShow">
				<installation-select :disabled="update" v-model="model.installation_id"></installation-select>
				<general-data v-model:name="model.name" v-model:last_name="model.last_name"
					v-model:phone_number="model.phone_number" v-model:position="model.position" v-model:dni="model.dni"
					v-model:email="model.email" :is-responsible="model.representative" />
				<div class="row border rounded border-light px-4 py-2 mt-3">
					<div class="col-12 border-bottom border-light mb-md-3">
						<h4>Documentacion</h4>
					</div>
					<div class="col-lg-6">
						<base-field apiName="date_firm" name="date_firm" label="Fecha de Alta">
							<field-validate type="date" class="form-control" name="date_firm"
								:rules="{ 'required': !$functions.empty(model.file_firm.file) }" label="fecha"
								v-model="model.date_firm" />
						</base-field>
					</div>
					<div class="col-lg-6">
						<base-field apiName="file_firm" name="file_firm" label="Documento de Alta">
							<div v-if="!$functions.empty(model.file_firm.file) && !update">
								<span class="mr-md-4">{{ model.file_firm.file[0].name }}</span>
								<base-button @click="model.file_firm.file = []" size="sm" type="default"
							...
							<div v-else-if="update && file_firm && !new_firm_doc">
								...
								<base-button @click="new_firm_doc = true" size="sm" type="default" :outline="true"><i
										class="fa-solid fa-pencil"></i></base-button>
							</div>
							<field-validate v-else type="file" class="form-control" name="file_firm"
								:rules="{ 'required': !$functions.empty(model.date_firm) }" label="documento"
					...
					<div class="col-lg-12">
						...
						<div class="row">
							<div class="col-lg-6">
								<base-field apiName="date_certification" name="date_cer" label="Fecha de formación">
									<field-validate type="date" class="form-control" name="date_cer"
										:rules="{ 'required': !$empty(model.file_certification.file) }" label="fecha"
										:disabled="model.last_formation?.dg_formation"
										v-model="model.date_certification" />
								</base-field>
							</div>
							<div class="col-lg-6">
								<base-field apiName="file_certification" name="file_cer" label="Documento de formación">
									<div
										v-if="(model.last_formation && !model.last_formation?.dg_formation) || !model.last_formation">
										<div v-if="!$functions.empty(model.file_certification.file) && !update">
											<span class="mr-md-4">{{
													model.file_certification.file[0].name
											}}</span>
											<base-button @click="model.file_certification.file = []" size="sm"
												type="default" :outline="true"><i class="fa-solid fa-pencil"></i>
											</base-button>
										</div>
										<div v-else-if="update && file_cer && !new_cer_doc">
											...
											<base-button @click="new_cer_doc = true" size="sm" type="default"
												:outline="true"><i class="fa-solid fa-pencil"></i></base-button>
										</div>
					...
					<div class="col-lg-12">
						<div class="row">
							<div class="col-lg-2">
								<base-switch v-model="model.driver" label="Conductor" :disabled="driver"
									:value="model.driver != 0 ? true : false"></base-switch>
							</div>
							<div class="col-lg-3" v-if="model.driver">
								<base-field apiName="phone_number" name="adr_permit_id" label="Permiso ADR">
									<field-validate as="select" class="form-control" name="adr_permit_id"
										v-model="model.adr_permit_id" rules="required">
							...
							<div class="col-lg-3" v-if="model.driver">
								<base-field apiName="driver_document_date" name="driver_date"
									label="Caducidad Carnet ADR">
									<field-validate type="date" class="form-control" name="driver_date"
										v-model="model.driver_document_date" rules="required"
										label="Caducidad Carnet ADR">
									</field-validate>
								</base-field>
							</div>
							<div class="col-lg-4" v-if="model.driver">
								<base-field apiName="file_driver" name="file_driver" label="Documentacion ADR">
									<div v-if="!$functions.empty(model.driver_document.file) && !update">
										<span class="mr-md-4">{{
												model.driver_document.file[0].name
										}}</span>
										<base-button @click="model.driver_document.file = []" size="sm" type="default"
											:outline="true"><i class="fa-solid fa-pencil"></i></base-button>
									</div>
									<div v-else-if="update && file_driver && !new_driver_doc">
										...
										<base-button @click="new_driver_doc = true" size="sm" type="default"
											:outline="true"><i class="fa-solid fa-pencil"></i></base-button>
									</div>
									<field-validate v-else type="file" class="form-control" name="file_driver"
										rules="required" label="documento" v-model="model.driver_document.file" />
					...
					<div class="col-12">
						<base-switch v-model="model.representative" :value="model.representative != 0 ? true : false"
							label="Representante de la instalación"></base-switch>
					</div>
				</div>
				<div class="mt-4 flex justify-end">
					<base-button v-if="update" type="default" nativeType="submit">Actualizar</base-button>
					<base-button v-if="!update" type="default" :disabled="!meta.valid" nativeType="submit">Aceptar
					</base-button>
					<base-button type="default" :outline="true" @click="handleClose(resetForm)">Cancelar
					</base-button>
				</div>
			</form-validate>
			<div v-else>
...
import EmployeeTrainingTable from '../Training/EmployeeTrainingTable.vue';
import GeneralData from './Modules/GeneralData.vue';
import FormSectionLoader from '../../loaders/FormSectionLoader.vue';

export default {
	components: { InstallationSelect, EmployeeTrainingTable, GeneralData, FormSectionLoader },
	name: "form-employee",
	mixins: [utils],
	props: {
		installation_id: {
