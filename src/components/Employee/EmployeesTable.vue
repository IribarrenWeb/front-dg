<template>
	<div :class="`card shadow ${classes}`">
		<div class="card-header border-0">
			<div class="row align-items-center">
				...
				<div class="col text-right">
					<a href="#" class="btn btn-sm btn-default" @click="handleAdd">Agregar</a>
				</div>
			</div>
		</div>
		...
		<div class="table-responsive">
			<base-table thead-classes="thead-light" :data="tableData">
				...
				<template v-slot:default="row">
					...
					<td class="d-flex">
						<a href="#" class="btn btn-sm btn-default" @click="employee_id = row.item?.id, modal = true"><i
		...
		<modal v-if="this.modal" v-model:show="this.modal" :action="action"
			@close="action = 'registrar', employee_id = null" modalClasses="modal-xxl" model="empleado">
			<form-employee @close="this.modal = false, employee_id = null" @reload="index(), this.$emit('reload_dash')"
