		...
		<form-validate ref="form" @submit="onSubmit" v-slot="{ resetForm, meta }">
			...
			<template v-if="currentStep == 1">
				<general-data v-model:file_document="model.file_document" v-model:file_auditor="model.file_auditor"
					...
					v-model:periodicity="model.periodicity" :delegate_id="delegate_id" :isSaved="isSaved" v-model:ministry_id="model.ministry_id" />
								...
								<div class="col-lg-6">
									<base-field name="operations" label="Tipos de operaciones">
										<div v-for="operation in operations" :key="operation.key" class="form-check">
											<field-validate :disabled="isSaved" type="checkbox" name="operations"
												class="form-check-input" :value="operation.value"
												v-model="model.operation_types_ids" rules="" label="operaciones">
											</field-validate>
								...
								<div class="col-lg-6">
									<base-field name="deposits" label="Tipos de depósitos">
										<div v-for="deposit in deposits" :key="deposit.key" class="form-check">
											<field-validate :disabled="isSaved" type="checkbox" name="deposits"
												class="form-check-input" :value="deposit.value"
												v-model="model.deposit_types_ids" rules="" label="depósitos">
											</field-validate>
											<label for="" class="form-check-label">{{
											deposit.label
											}}</label>
						...
						<div class="col-lg-12">
							<base-field name="equipments" label="Tipos de equipamientos" labelClasses="d-block">
								<div v-for="equipment in equipments" :key="equipment.key" class="d-inline-block">
									<field-validate :disabled="isSaved" type="checkbox" name="equipments"
										:value="equipment.value" v-model="model.equipments_ids" rules=""
										label="equipamientos">
									</field-validate>
			...
			<template v-if="currentStep == 3">
				<div v-if="!isSaved && (modalResponsible || selectedResponsible)">
					<employee-selector v-model:show="modalResponsible" @selected="selectedResponsible = $event"
						:business_id="business_id" />
					<div v-if="selectedResponsible">
						<base-button @click="selectedResponsible = null" class="mb-4">Crear responsable</base-button>
						<employee-general-data :name="selectedResponsible.name"
							:last_name="selectedResponsible.last_name" :phone_number="selectedResponsible.phone_number"
							:position="selectedResponsible.position" :dni="selectedResponsible.dni"
							:email="selectedResponsible.email" :noEdit="true" />
					</div>
				</div>
				<div v-else>
					<div v-if="!isSaved">
						...
						<employee-general-data custom_key="responsible" v-model:name="model.responsible.name"
							...
							v-model:position="model.responsible.position" v-model:dni="model.responsible.dni"
						...
						<div class="row border rounded border-light px-4 py-2 mt-3">
							...
							<div class="col-lg-6">
								<base-field apiName="responsible.date_firm" name="date_firm" label="Fecha de Alta">
									<field-validate :disabled="isSaved" type="date" class="form-control"
										name="date_firm" :rules="{
											required: !$functions.empty(
												model.responsible.file_firm.file
											),
										}" label="fecha" v-model="model.responsible.date_firm" />
								</base-field>
							</div>
							<div class="col-lg-6">
								<base-field apiName="responsible.file_firm" name="file_firm" label="Documento de Alta">
									<div v-if="model.responsible.file_firm.file.length >= 1">
										...
										<base-button @click="model.responsible.file_firm.file = []" size="sm"
											type="default" :outline="true" :disabled="isSaved"><i
												class="fa-solid fa-pencil"></i>
										</base-button>
									</div>
									<field-validate :disabled="isSaved" v-else type="file" class="form-control"
										name="file_firm" :rules="{
											required: !$functions.empty(model.responsible.date_firm),
										}" label="documento" v-model="model.responsible.file_firm.file" />
								</base-field>
							</div>
							<!-- --------------------------------------- -->
							<div class="col-lg-12">
									...
									<div class="col-lg-6">
										<base-field apiName="responsible.date_certification" name="date_cer"
											...
											<field-validate :disabled="isSaved" type="date" class="form-control"
												...
												}" label="fecha" v-model="model.responsible.date_certification" />
										</base-field>
									</div>
									<div class="col-lg-6">
										<base-field apiName="responsible.file_certification" name="file_cer"
											...
											">
												...
												<base-button @click="model.responsible.file_certification.file = []"
													size="sm" type="default" :outline="true" :disabled="isSaved"><i
							...
							<div class="col-lg-12" v-if="isTransported">
								<div class="row">
									<div class="col-lg-2">
										<base-switch :disabled="isSaved" v-model="model.responsible.driver"
											label="Conductor" :value="model.responsible.driver != 0 ? true : false">
										</base-switch>
									</div>
									<div class="col-lg-3" v-if="model.responsible.driver">
										<base-field apiName="responsible.phone_number" name="adr_permit_id"
											label="Permiso ADR">
											<field-validate :disabled="isSaved" as="select" class="form-control"
												name="adr_permit_id" v-model="model.responsible.adr_permit_id"
												rules="required">
												<option selected>Permiso adr</option>
												<option :value="permit.id" v-for="(permit, key) in permits"
													:key="key.id">
													{{ permit.name }}
												</option>
											</field-validate>
									...
									<div class="col-lg-3" v-if="model.responsible.driver">
										<base-field apiName="responsible.driver_document_date" name="driver_date"
											label="Caducidad Carnet ADR">
											<field-validate :disabled="isSaved" type="date" class="form-control"
												name="driver_date" label="Caducidad Carnet ADR"
												v-model="model.responsible.driver_document_date" rules="required">
											</field-validate>
										</base-field>
									</div>
									<div class="col-lg-4" v-if="model.responsible.driver">
										<base-field apiName="responsible.file_driver" name="file_driver"
											...
											<div v-if="model.responsible.driver_document.file.length >= 1">
												...
												<base-button @click="model.responsible.driver_document.file = []"
													size="sm" type="default" :outline="true" :disabled="isSaved"><i
														class="fa-solid fa-pencil"></i></base-button>
											</div>
											<field-validate :disabled="isSaved" v-else type="file" class="form-control"
												name="file_driver" rules="required" label="documento"
			...
			<div class="mt-4 float-md-right">
				...
				<div class="d-inline-block mx-2">
					<base-button type="default" :disabled="!meta.valid" nativeType="submit"
						v-if="currentStep !== 3 && currentStep < 6">Siguiente</base-button>
				</div>
				<base-button type="default" :disabled="!meta.valid" nativeType="submit" v-if="currentStep === 3">
					{{ !isSaved ? "Aceptar" : "Siguiente" }}</base-button>
				<base-button type="default" :outline="true" class="ml-auto" @click="handleClose(resetForm)">{{ isSaved ?
				"Cerrar" : "Cancelar" }}
				</base-button>
			</div>
		</form-validate>
...
import BaseButton from "../core_components/BaseButton.vue";
		...
		async onSubmit(values) {
			...
			if (this.currentStep == 3 && !this.isSaved) {
				...
				try {
					...
					let data = this.$functions.cleanData(this.model);
					
					if (this.selectedResponsible) {
						data = {
							...data,
							selected_responsible: this.selectedResponsible.id
						}
					}
