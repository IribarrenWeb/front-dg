<template>
	<div class="card mb-md-5">
		...
		<div class="card-header border-0 pl-2 py-3 bac-ligth mx-0 row align-items-end filter-container">
			<div class="col-md-10">
				<div class="row align-items-end">
					<date-filter class="col-12 col-md-4 col-lg-3" v-model:clear="clear"
						@updated="handleFilter('date', $event)" />
					<div class="col-12 col-md-4  mt-2 mt-md-0 col-lg-3 filter" v-if="!$store.state.is_auditor">
						<async-select placeholder="Selecciona responsable..." v-model:clear="clear"
							@updated="handleFilter('responsible', $event.id)" v-if="!$store.state.is_auditor"
							:roles="[2, 3]"></async-select>
					</div>
					<business-filter class="col-12 col-md-4 mt-2 mt-md-0 col-lg-3" v-model:clear="clear"
						@updated="handleFilter('business', $event)"
						v-if="$store.state.is_auditor || $store.state.is_delegate" />
					<installation-filter class="col-12 col-md-4 mt-2 mt-md-0 col-lg-3" v-model:clear="clear"
		...
		<div class="table-responsive">
			<base-table thead-classes="thead-light" :data="tableData">
				...
				<template v-slot:default="row">
					...
					<td>
						<badge class="badge-dot mr-4 text-capitalize" :type="row.item?.status ? 'success' : 'danger'">
			...
			<modal v-if="modal && selected_training" modalClasses="modal-xxl"
				:action="selected_training.status ? 'detalle' : reAssign ? 're-asignar' : 'completar'" v-model:show="modal"
				model="Formación">
				<form-complete-formation :toComplete="!reAssign" :installation_id="selected_training.installation_id"
					:reAssign="reAssign" :isComplete="selected_training.status" :training_id="selected_training.id"
					@close="modal = false, selected_training = null, reAssign = false" @reload="index" />
			</modal>
		</div>
	</div>
</template>
